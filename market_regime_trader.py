...
#    ── 이번 리포트 판정 ── R91이 예상보다 잘 됐다: S★ 69.2%·92.3% · I★ 68.9%·95.9%(둘 다 중간 · 높음까지 회피 0.8·1.1%p).
#      R91 사전등록 네 조건(회피 ≥68 · 참여 ≥90 · MDD 악화 ≤0.3 · 배수 ≥17.5) 전부 통과. 블록 G(일별 FF): R91 사이징이 R88보다
#      **네 시대 모두 나았다**(1927~49 +0.22 · 1950~98 +0.48 · 1999~2017 +0.57 · 2018~ +1.19%p). R86 대비는 여전히 반증(3회째).
#    ── ⚠ 새로 확인한 사실 ── 같은 코드를 다시 돌려도 M 복합점수가 633일에서 달라졌다(SPY 가격은 동일 → 상류 지표 자료 변동).
#      이 때문에 R91 예상(68.3/90.5)보다 엔진(69.2/92.3)이 +0.9/+1.8%p 높았다 — **실행마다 회피·참여가 1~2%p 흔들린다.**
#    ── 이번 변경(⚠ 위험 파라미터 1개 · M) ── EXTENSION_HAIRCUT_STEPS ((0.10,0.6),(0.12,0.0)) → ((0.10,0.55),(0.12,0.0)).
#      S 중립채움 1.0→0.5와 한 묶음 — 하네스 S★ 70.47/90.41 · I★ 70.24/93.90(둘 다 높음) · 노출 0.569→0.544.
#      ⚠ 여유가 0.2%p뿐이라 위 흔들림으로 다음 실행에서 중간으로 돌아갈 수 있다(사용자에게 알리고 골랐다).
#      ⚠ 되돌리기: m_overrides={"EXTENSION_HAIRCUT_STEPS": ((0.10, 0.6), (0.12, 0.0))}.
#    ── 로그 ── [CONFIG] event=r92_sizing is_r92=True changed_vs_r91=True.
#    연구·교육용이며 투자 자문이 아니다.
...
#    시작 v1.57.0 → 목표 v1.58.0. 변경 함수 없음 — Config 값 2개만(generate_signals 규칙 ⑩·pos_map이 그대로 읽는다).
#    (a) EXTENSION_HAIRCUT_STEPS ((0.10,0.6),(0.12,0.4)) → **((0.10,0.4),(0.12,0.2))** [E7 → E11 · 과열 헤어컷 강화 = 회피 손잡이]
#        근거(엔진 s16 · 2018~ · 신호일 t → t+1 SPY 로그수익 합): 확정 RISK_ON & 이격 ≥12%(현 E=0.4) 262일은 상승구간 +17.4 ·
#        하락구간 −24.5로 **순음수**(하락이 더 크다) → 더 줄이면 회피가 참여보다 더 오른다. S 층 [M헤어컷격자] E11 행(엔진 실측):
#        회피 63.6→67.7% · 참여 83.7→81.5% · 배수 12.77→13.43 · 칼마 3.888→3.977 · MDD 동일.
#        독립 구간(2000~2017 · SPX 일별 + M 대용 3상태 · 오프라인 r88/long88.py): 헤어컷을 깊게 할수록 회피+참여 합이 두 반쪽
#        (2000~08 · 2009~17) 모두 조금씩 오른다(E7→E11 +0.009 · +0.003) · MDD는 개선(−38.07 → −36.97%).
#    (b) POS_NEUTRAL 0.50 → **0.60** [중립 국면 비중 = 참여 손잡이]
#        근거(엔진 s16): 중립 & E=0.5 258일은 상승구간 +31.5 · 하락구간 −23.6(비 1.33). 하네스 S★: 회피 −0.9%p · 참여 +1.5%p.
#        과거 격자(v1.33.0 주석): 0.6 CAGR +0.19pp · MDD −0.51pp. 독립 구간: 2000~08 합 +0.013 · 2009~17 −0.003(섞임 — ⚠ 약한 근거).
#    합산 효과(하네스 · 양쪽형 = M 두 줄 + S 상한 0.9·방어대피처 0·하락국면리더 0): S★ 회피 63.4→68.1% · 참여 83.4→88.1% ·
#      배수 12.5→16.5 · MDD −8.75→−10.05% · 칼마 3.85→3.79. **높음(70%·90%) 미도달 — 부족 회피 1.9%p · 참여 1.9%p.** M 단독 하네스: 회피 70.2→72.3 ·
#      참여 63.2→61.9 · 배수 +2.4% · 칼마 2.548→2.586.
#    ⚠ 영향 범위: M의 SPY 목표비중(실매매 근거) · S/I/K가 받는 E_t · 섹터 자기 기계(sector_cfg_for가 M cfg를 복제 — S★ 배분은
#      섹터 target_pos를 읽지 않으므로 무영향, 단일 섹터 예측 행만 바뀐다) · 워크포워드 캐시 키(cfg 지문 — 전 경계 재계산).
#    ⚠ 되돌리기(한 줄): m_overrides={"EXTENSION_HAIRCUT_STEPS": ((0.10, 0.6), (0.12, 0.4)), "POS_NEUTRAL": 0.5} ⇒ v1.57.0과 비트 동일.
#    로그: [CONFIG] event=r88_sizing(시작 시 1회) · [SIGNAL] 기존 헤어컷 발동일 로그가 새 상한(0.4/0.2)으로 찍힌다.
#    연구·교육용이며 투자 자문이 아니다.
...
#
#                        [§1 리포트58 판정 — M 격자는 두 라운드 연속 수렴]
#                        06c 121행 중 강건성 분포: 열위(강건) 76 · ★ 28 · ⚠ 5일 의존 13 · 통과 7 · nan 4.
#                        기준 ④(강건성='통과')를 넘은 7행 중 **①②③까지 통과한 행은 0개**.
#                        헤어컷 사다리는 E3부터 MDD -0.0707 · 상위3 낙폭평균 -0.0685로 **완전 포화**이고
#                        E9·E11·E13·E9-바닥0은 전부 `⚠ 5일 의존`(초과 +0.125/+0.245/+0.365/+0.501 →
#                        5일컷 -0.087/-0.179/-0.272/-0.236)이라 ④-a 불통과다. v1.47.0의 E7 유지가 재확인됐다.
#                        성과는 리포트57과 동일: CAGR 25.68% · 샤프 2.25 · MDD -7.07% · 칼마 3.633.
#
#                        [§2 그래서 무엇을 고쳤나 — 세는 일을 사람이 하지 않게 한다]
#                        지난 다섯 라운드 모두 "①②③④ 전부 통과한 행이 있나"를 손으로 세어 판정했다.
#                        그 계산을 리포트가 스스로 하게 `_grid_convergence_line(res)`(신규, 모듈 전역)를
#                        만들고 00_실행요약에 **"격자 수렴 상태(①②③④)"** 한 줄을 넣었다. 출력 예:
#                          격자 121행 — ① 통과 N · ④ 통과 7 · **①②③④ 전부 통과 0행** (후보: 없음) |
#                          ★ CAGR 25.68% 칼마 3.633 MDD -7.07% · 격자 칼마 정점 3.728 — 0행이면 채택 후보 없음
#                        ⚠ 아님 — 06c를 읽기만 하고 신호·배분·파라미터는 한 줄도 건드리지 않는다.
#                        후보가 생기면 그 행의 파라미터 열(값이 채워진 열)로 어느 손잡이인지까지 찍는다.
#
#                        [§3 섹터층에서 나온 이번 라운드의 진짜 발견 — M에도 해당된다]
#                        섹터 ★의 최대낙폭 -9.75%는 2019-04-24~06-19인데 그 40일 내내 M은
#                        **상승(위험선호) · E_t=1.00**이었고 H 백분위 최고 0.64였다. 구간 누적은
#                        SPY +0.06% vs XLK -1.31%. 즉 **국면 판단이 틀린 사건이 아니다.**
#                        M 자신의 같은 구간 낙폭은 -6.61%. 이 사실은 "낙폭을 더 줄이려면 H·국면에
#                        연동한 조건부 축소를 넣자"는 계열의 처방이 왜 매번 기각되는지를 설명한다 —
#                        줄여야 할 낙폭이 **모델이 옳게 '상승'이라고 부른 구간 안에** 있기 때문이다.
#                        (섹터층 실증: 상대강도 가드 18개 변형 전부 ★보다 나쁨 — CHANGELOG_SECTOR v0.36.0 §C)
#
#                        변경 모듈: `_grid_convergence_line()`(신규), 00_실행요약 meta 1줄, 버전 문자열 2곳.
#                        회귀: test_grid_convergence_v149.py 신규 + 기존 44개 재실행.
#
#  VERSION: v1.48.0 - 2026-09-11 - [격자 수렴 확인 — **활성 신호 변경 없음**. 낙폭 형태 2열 추가
#                       + 기준 ④-b(i) 보강(포화는 **수용기준이 쓰는 지표(MDD)** 로 판정)]
#                       변경 모듈: `_grid_robust()`(상위3 낙폭평균·얼서지수 2열 추가, 문서).
#                       **신호 로직·채택값 전부 무변경 — v1.47.0과 비트 동일.**
#
#                       [§0 v1.47.0 검증 — 네 지점 전부 통과, 예측이 거의 그대로 맞았다]
#                       ① 섹터 **13f ② PASS 복귀**: 악화 +0.23%p(예측 0.2~0.3%p)
#                       ② 섹터 ★ **MDD -9.75%**(예측 -9.75%) · 칼마 **3.669**(예측 3.6~3.7) ·
#                          CAGR **35.78%**(예측 35.71%)
#                       ③ `[M헤어컷격자·근사]` 정상 출력. 예측 대 실측(섹터 MDD):
#                          off -15.50/-15.50 · E1 -13.37/-13.37 · E3 -11.21/-11.20 ·
#                          E5 -9.85/-9.85 · E9 -9.75/-9.75 · E11 -9.75/-9.75 — **소수점까지 일치.**
#                          **E9·E11이 E7(★)과 같은 -9.75%** → 포화 재확인. 게다가 두 행은
#                          `⚠ 5일 의존`(+0.281 → -0.014 / +0.557 → -0.033)이라 ④-a도 불통과다.
#                       ④ DEEP -12% 여전히 `⚠ 5일 의존`(+0.268 → -0.385) — 보류 유지.
#
#                       [§A M 격자는 수렴했다 — 이번 라운드 채택 후보 없음]
#                       06c에서 기준 ④를 통과한 행은 7개이고 **전부 레버리지·노출배수**다.
#                       그중 MDD 악화가 가장 작은 '전 비중 ×1.5'조차 칼마 3.539로 ★(3.633)에
#                       못 미쳐 기준 ②에서 탈락한다(나머지는 기준 ③에서 탈락, 악화 +1.74~+6.92%p).
#                       **①②③④를 모두 통과하는 행이 하나도 없다.**
#
#                       [§B 왜 수렴했나 — M의 MDD는 예보 불가능한 하루짜리 갭이 정한다]
#                       M의 -7.07%는 **2020-06-09~11 3거래일** 사건이고, 그 중 **6/11 하루가
#                       -5.85%** 다(그날 체결비중 1.0 · 국면 '상승(위험선호)' · 어떤 규칙도 침묵).
#                       독립 낙폭 에피소드 상위 5개는 -7.07 / -6.88 / -6.61 / -6.04 / -5.26%로
#                       **넓게 분포**한다 — MDD가 단일 이상치에 끌린 값이 아니라 **구조적 바닥**이다.
#                       느린 신호로 사이징하는 어떤 규칙도 이 바닥을 내리지 못하므로 격자가 전부
#                       여기서 포화한다. 이것이 §A의 원인이다.
#
#                       [§C ⚠ 기준 ④-b(i) 보강 — '어떤 낙폭 지표로 포화를 보느냐'가 손잡이가 되면 안 된다]
#                       같은 사다리도 **낙폭 지표를 넓힐수록 포화점이 뒤로 밀린다**(섹터 M헤어컷 사다리):
#                          MDD **E7** 포화 · 상위3 낙폭평균 **E9** 포화 · 상위5 **E11** · **얼서는 무한**
#                       지표를 고를 자유를 두면 "포화했다"가 그 자체로 튜닝 가능해지고, 극단에서는
#                       노출 0이 정답이 된다. → **포화는 수용기준이 실제로 쓰는 지표(MDD)로 판정한다.**
#                       13f ②도 기준 ③도 MDD를 쓰므로 일관된다. `상위3 낙폭평균`은 "MDD가 단일
#                       사건에 고정돼 헛포화한 것은 아닌가"를 교차 확인하는 **보조**, `얼서지수`는
#                       **맥락 전용**이다. 두 열을 06c 전 행과 섹터 13 시트에 상설로 실었다.
#                       이 기준으로 **E7 유지**가 재확인된다(E9는 MDD 포화 이후 + ④-a 불통과).
#
#                       [§D 새 기각 — 중립일 추가 감축(축 하나 닫음)]
#                       12_예측정확도 A블록에서 중립 상태는 익일 적중이 기저 대비 **-4.06%p(역방향)**,
#                       익일 평균 **-3.97bp**다. 규칙 ⑨가 닿지 않는 **중립 & H≤0.70 316일**을 전수
#                       분해하니 익일 초과 **-2.00bp, t=-0.38**(사실상 0)이고 H 구간별 단조성도 없었다.
#                       실제로 깎아보면(리플레이):
#                          중립 전부 0     CAGR 23.97%(-1.60%p) MDD -7.07%(불변) 상위3 -6.85%(불변)
#                          중립 전부 0.25  CAGR 24.78%(-0.79%p) 〃                 〃
#                          중립 전부 0.4   CAGR 25.27%(-0.30%p) 〃                 〃
#                          중립 & H≤0.60→0 CAGR 25.24%(-0.33%p) 〃                 〃
#                       **전부 상위 5일을 제외해도 음수**(-0.40~-2.01%p)라 ④-a 불통과이고,
#                       **MDD·상위3이 한 자리도 움직이지 않아** ④-b도 불통과다. → 기각. 축을 닫는다.
#                       (즉 현행 '0 @ H>0.70'이 이 축의 올바른 지점이다.)
#
#                       [§E 다음 리포트에서 확인할 것]
#                       ① 06c·13 시트에 `상위3 낙폭평균`·`얼서지수`가 실렸는가.
#                       ② `[M헤어컷격자]`의 E9·E11이 계속 E7과 같은 MDD인가(포화 유지).
#                       ③ 기준 ①②③④를 모두 통과하는 행이 생겼는가 — 생기면 그때 채택한다.
#
#  VERSION: v1.47.0 - 2026-09-11 - [⚠⚠ E3 → E7 — v1.46.0 되돌림의 되돌림. 그 분석에
#                       **측정 범위의 구멍**이 있었다 + 기준 ④를 ④-a/④-b로 정밀화]
#                       변경 모듈: `Config.EXTENSION_HAIRCUT_STEPS`((0.10,0.8),(0.12,0.6) →
#                       **((0.10,0.6),(0.12,0.4))**), `_grid_robust()` 문서.
#                       **활성 신호 변경은 EXTENSION_HAIRCUT_STEPS 단독**(v1.4.0 §9).
#
#                       [§0 v1.46.0 검증 — 게이트는 제대로 작동했고, 그 덕에 구멍도 드러났다]
#                       ① 06c·13 시트에 강건성 3열 정상 출력, ★ 행 0.000 ✅
#                       ② **DEEP -12%가 '⚠ 5일 의존'(+0.167 → -0.427)** — 게이트가 걸러냈다.
#                          기준 ④가 없었다면 이번 라운드에 채택했을 후보다. 게이트의 첫 성과다.
#                       ③ 헤어컷 E5~E13·바닥0 전부 '⚠ 5일 의존', off·E1은 '열위(강건)' ✅
#                       ④ E3 되돌림 실측 CAGR **25.26%** / 칼마 **3.574**(예측 25.30 / 3.579) ✅
#                       06c 전체에서 기준 ④를 통과한 행은 6개뿐이고 **전부 레버리지·노출배수**라
#                       기준 ③(MDD 악화 ≤0.5%p)에 걸린다 → **M 격자는 수렴했다.**
#
#                       [§A ⚠⚠ 그런데 섹터 층에서 사고가 났다 — 13f ②가 FAIL로 넘어갔다]
#                       리포트34: 섹터 ★ MDD **-9.75% → -11.20%**, 칼마 3.697 → 3.112,
#                       13f 수용기준 ②(대조군A 대비 MDD 악화 ≤ 1%p)가 **1.68%p로 FAIL**.
#                       원인은 하나다 — **E3 되돌림으로 M의 E_t가 커졌고, 이 층은 그것을 XLK
#                       베타(~1.3)로 증폭한다.** 최대낙폭 구간도 2019-05에서
#                       **2024-07-10~08-07(엔캐리 청산, 21거래일)** 로 옮겨갔다(섹터 -10.59% vs M -5.04%).
#
#                       [§B 내 v1.46.0 분석의 구멍 — 변하지 않는 숫자로는 효과를 잴 수 없다]
#                       나는 사다리를 **M 단독 지표로만** 검정했다. 수익 쪽 결론(5일 의존)은 지금도
#                       맞다. 그런데 **낙폭 쪽은 M에서 잴 수가 없었다** — M의 MDD는 2020 코로나
#                       구간이 고정하고 있고 그 구간은 이격 국면이 아니라서 헤어컷이 닿지 않는다
#                       (**E3~E13 전부 -7.07%로 완전히 동일**, E1만 -7.27%). 상수를 보고 "효과 없음"이라 읽었다.
#                       리포트56 자료로 같은 사다리를 **섹터 층**에서 재현했더니:
#                         M헤어컷   섹터 CAGR  샤프    섹터 MDD    칼마    2024-07~08   13f ②
#                         off       32.89%   1.829  **-15.50%** 2.122   -14.92%
#                         E1        33.85%   1.960   -13.37%   2.532   -12.77%
#                         E3(v1.46) 34.78%   2.088   -11.21%   3.104   -10.59%    **1.69%p FAIL**
#                         E5        35.26%   2.166    -9.85%   3.578    -9.23%     0.34%p PASS
#                         E7        35.71%   2.238  **-9.75%** 3.662    -7.87%     0.23%p PASS ← 채택
#                         E9        36.15%   2.305    -9.75%   3.707    -6.50%     0.23%p PASS
#                         E11       36.57%   2.364    -9.75%   3.751    -5.11%
#                       근사 검증: 이 방식으로 만든 E9 행(36.15% / -9.75% / 3.707)이 E9가 라이브였던
#                       **리포트33 실측(36.05% / -9.75% / 3.697)과 거의 일치**한다.
#
#                       [§C 왜 E7에서 멈추는가 — 이번에는 포화점이 있다]
#                       E7부터 섹터 최대낙폭 **구간이 바뀐다**: 2024-07-10~08-07 → 2019-04-23~06-03.
#                       그리고 E7·E9·E11이 전부 **-9.75%로 완전히 같다** — 헤어컷이 걷어낼 수 있는
#                       낙폭을 다 걷어낸 지점이다. v1.40~v1.45의 CAGR 사다리는 **끝없이 올라가
#                       멈출 곳이 없었고**, 이 낙폭 사다리는 **정확히 E7에서 멈춘다.**
#                       그 차이가 이번 채택과 지난 세 번의 채택을 가른다.
#                       ⚠ 수익 근거는 여전히 없다(E7 ★대비 +0.579 → 상위5일 제외 -0.011).
#                       **채택 근거는 오직 낙폭이고, CAGR은 잃지도 얻지도 않는 것으로 본다.**
#
#                       [§D 기준 ④ 정밀화 — ④-a(수익) / ④-b(낙폭)]
#                       ④-a 수익 근거 채택: 초과수익 부호가 상위 5일 제외 후에도 유지될 것.
#                       ④-b 낙폭 근거 채택: (i) 낙폭 개선이 단조이고 **격자 안에서 포화**할 것
#                            (ii) 초과수익이 상위 5일 제외 후 음수가 아닐 것
#                            (iii) **실제로 운용하는 층에서 측정할 것** — E_t처럼 두 층이 공유하는
#                            파라미터는 상위 층 단독 지표가 구조적으로 둔감할 수 있다.
#                       (iii)이 이번 라운드의 교훈이다. S v0.34.0의 `[M헤어컷격자·근사]`가 매 실행
#                       이 판정을 자동으로 보여준다.
#
#                       [§E 다음 리포트에서 확인할 것]
#                       ① 섹터 13f ②가 **PASS로 복귀**했는가(예상 악화 0.2~0.3%p).
#                       ② 섹터 ★ MDD가 -9.75% 부근인가 · 칼마 3.6~3.7인가.
#                       ③ `[M헤어컷격자·근사]`가 실렸는가 — E9·E11 행이 E7과 **같은 MDD**인가
#                          (포화 재확인). 다르면 포화점을 다시 잡아야 한다.
#                       ④ DEEP -12%의 강건성이 여전히 '⚠ 5일 의존'인가.
#
...
#                       `_grid_accuracy()`(노출가중 3열 추가), 06c 헤어컷 격자(+E11·E13·바닥0),
#                       06c DEEP_RECOVERY_DD 격자(+-10%·-8%).
#                       **활성 신호 변경은 EXTENSION_HAIRCUT_STEPS 단독**(v1.4.0 §9).
#
#                       [§0 v1.44.0 검증 완료 — 예고한 확인 지점 네 개 전부 통과]
#                       ① **국면정의 검증 PASS** — 위험선호 익일평균 0.1149% > 위험회피 0.0443%.
#                          좁아진 밴드에서도 국면 정의가 유효하다(되돌림 조건 불발).
#                       ② **0.10은 격자의 끝이 아니라 내부 최적점이었다.** 리포트54 실측:
#                            0.00 CAGR 24.48% 칼마 3.255 MDD **-7.52%**  (매크로 위험회피 밴드 끔)
#                            0.05 CAGR 24.57% 칼마 3.275 MDD **-7.50%**
#                            0.10 CAGR **25.50%** 칼마 **3.607** MDD -7.07%  ← ★
#                            0.15 CAGR 25.13% 칼마 3.555 MDD -7.07%
#                            0.20 CAGR 24.01% 칼마 3.397 MDD -7.07%
#                          아래로 더 내려가면 CAGR·샤프·칼마·MDD가 **동시에** 나빠진다.
#                          즉 매크로 위험회피 밴드는 좁게라도 값을 한다 — 규칙을 지울 수 없다.
#                       ③ **DEEP 격자 ★ 행이 라이브와 일치**(25.50%/3.607) — v1.44.0 §D 수정 확인.
#                       ④ 추세승격 0.20('0.10+버퍼 0.10' 등가)은 **기각** — CAGR 24.10% ·
#                          칼마 2.708 · MDD -8.90%. 현행 0.30 유지(이 축도 닫혔다).
#                       예측 대비 실측: CAGR 25.48%(예측) → **25.50%**, 샤프 2.192 → **2.194**,
#                       MDD -7.07% 동일, 놓친 큰상승 59 → **55일**.
#
#                       [§A ⚠ 규칙 ⑩ 과열 헤어컷 E5 → E9]
#                       v1.44.0이 "새 라이브 위에서 다시 측정돼야 한다"고 미룬 판정을 리포트54가 냈다.
...
#                         E9   CAGR **25.86%** 샤프 **2.300** 칼마 **3.659** MDD -7.07%  ← 채택
#                       MDD가 E3~E9 구간에서 -7.07%로 **완전히 고정**이다(최대낙폭 구간이 이격
#                       국면이 아니라 해어컷이 그 날들에 닿지 않는다). 기준 ①(CAGR +0.36%p)
#                       ②(06c 전체 칼마 정점) ③(MDD 악화 0.00%p) 전부 통과.
#                       ⚠ 대가: 평균노출 0.5884 → 0.5438 · 놓친 큰상승 55 → 57일 ·
#                       하락적중 +8.93 → +7.39%p(상승적중은 +3.17 → +4.57%p로 개선).
#
#                       [§B E9도 사다리의 끝이다 — E11·E13·'바닥 0'을 심는다]
#                       off→E1→E3→E5→E7→E9로 CAGR·샤프·칼마가 **단조 증가**한다. 정점인지
#                       사다리가 짧은 것인지 알 수 없으므로 v1.40.0/v1.44.0과 같은 원칙으로
#                       **E11(0.4/0.2) · E13(0.3/0.1) · E9-바닥0(0.5/0.0)** 세 행을 추가했다.
#                       특히 '바닥 0'은 "이격 +12% 이상이면 아예 들지 말라"는 극단이 이기는지를
#                       묻는다 — 이기면 규칙의 성격이 '감축'에서 '차단'으로 바뀌어야 한다는 뜻이다.
#
#                       [§C ⚠ 측정 결함 수정 — 이진 절단이 사이징 규칙을 구분하지 못했다]
#                       `_grid_accuracy()`의 적중률은 `pos_exec >= 0.5`로 보유/미보유를 **이진
#                       판정**한다. 리포트54에서 한계가 드러났다: **E7(상한 0.6/0.4)과
#                       E9(0.5/0.3)의 적중률 세 열이 소수점까지 동일**했다(상승 +4.57 ·
#                       하락 +7.39 · 놓친 57). 0.6과 0.5는 둘 다 '보유', 0.4와 0.3은 둘 다
#                       '미보유'로 같은 쪽에 떨어지기 때문이다. 0.1 단위 사이징 규칙(⑩ 해어컷 ·
#                       ⑨ 중립감축 · ⑮ 폭조절)을 이 열로 비교하면 **차이가 안 보이거나 절단
#                       위치가 바뀌는 쪽만 과장**된다. E5→E9의 하락적중 하락(8.93 → 7.39)도
#                       상당 부분 이 착시다.
#                       → 기존 열은 리포트 간 비교 연속성 때문에 **그대로 두고** 노출가중 3열을
#                         나란히 추가했다: 상승적중(노출가중,%p) · 하락적중(노출가중,%p) ·
#                         놓친 큰상승(노출가중,일). 상승쪽 분모는 투입자본, 하락쪽 분모는 현금이라
#                         **한쪽을 키우면 다른 쪽이 반드시 줄어든다**(노출만 낮추면 좋아지는
#                         정규화가 아니다). 앞으로의 판정은 두 판을 같이 본다.
#
#                       [§D 버그 수정이 바로 값을 했다 — DEEP -12%가 -15%(★)를 이긴다]
#                       v1.44.0 §D의 like-for-like 수정으로 이 격자가 처음 제대로 보였다.
#                         off   CAGR 19.66% 칼마 2.673 MDD -7.35%
#                         -12%  CAGR **25.75%** 칼마 **3.644** MDD -7.07% 하락적중 +8.95 놓친 55
#                         -15%  CAGR 25.50%   칼마 3.607   MDD -7.07% 하락적중 +8.93 놓친 55  ← ★
#                         -20%  CAGR 22.95%   칼마 3.247   MDD -7.07%
#                       기준 ①②③을 통과하는 후보지만 **한 커밋 한 신호 파라미터**(v1.4.0 §9)
#                       원칙으로 이번 라운드에는 채택하지 않는다. -12%가 격자의 끝이므로
#                       **-10%·-8%를 심어** 다음 실행이 정점을 확정하게 한다.
#
#                       [§E 기록 — v1.43.0의 구조 가정이 이번 변경으로 깨졌다]
#                       '⑦⑧만 제외' 행은 v1.43.0에서 ★와 여덟 지표가 전부 동일한 **구조적
#                       무효과**였다(⑦⑧은 RISK_ON 승격이라 ⑨의 대상인 확정 중립과 겹치지 않는다).
#                       리포트54에서는 **달라졌다**(CAGR 25.18% vs ★ 25.50%). 이유는 밴드 축소다:
#                       위험회피 밴드가 좁아지면서 원시상태 중립일이 늘고 → ⑦⑧ 승격일이 늘고 →
#                       그 중 H≥0.70인 날은 규칙 ②가 중립으로 되돌리므로 **⑨의 사정권에
#                       들어온다.** 두 집합이 겹치기 시작했다.
#                       그 행 자체는 여전히 ★보다 나쁘므로 채택 대상이 아니고, 이 항목은
#                       "구조 불변식이 깨지면 알려주는 canary가 제 일을 했다"는 기록으로 남긴다.
#                       회귀 테스트의 해당 검정은 **동일성 단정 → 변화 감지 시 사유 확인**으로 바꾼다.
#
#                       [§F 다음 리포트에서 확인할 것]
#                       ① E11·E13·바닥0 중 E9를 이기는 행이 있는가(있으면 사다리를 더 내린다).
#                       ② DEEP -10%·-8%가 -12%를 이기는가 → 그 축의 정점 확정 후 채택.
#                       ③ 노출가중 적중률이 이진 적중률과 어디서 갈리는가(§C의 착시 크기 확인).
#                       ④ 섹터 [방어대피처격자]가 새 E_t에서도 0.50을 정점으로 유지하는가.
#
#  VERSION: v1.44.0 - 2026-09-10 - [⚠⚠ 신호(위험) 변경 — 국면 위험회피 임계값 0.20 → 0.10.
#                       리포트 40~53 열네 번이 계속 내고 있던 답을 이제야 읽었다]
#                       변경 모듈: `Config.PCT_RISK_OFF`(0.20 → **0.10**), 06c 국면 임계값 격자
#                       (off 0.00·0.05 추가 · on 4→2 canary), 06c 추세승격 격자(+0.20),
#                       06c 헤어컷 격자(+E9), 06c DEEP_RECOVERY_DD 격자의 `struct_dd` 배선.
#                       **활성 신호 변경은 PCT_RISK_OFF 단독**(v1.4.0 §9 "한 커밋에 2개 이상 금지").
#
#                       [§A ⚠⚠ 이번 라운드 유일한 활성 변경 — PCT_RISK_OFF 0.20 → 0.10]
#                       06c 첫 블록(국면 임계값 격자)은 원래 **곡선맞춤 점검용**으로 만든 것이라
#                       ("특정 값에서만 좋으면 곡선맞춤이다") 채택 후보 목록으로 읽지 않았다.
#                       그런데 리포트 40~53을 한 줄로 세워보니 이 격자는 점검이 아니라 **열네 번
#                       같은 답을 내고 있었다**:
#                         위험회피 백분위   0.10     0.15     0.20(★)  0.25     0.30     0.35
#                         report45  CAGR  22.96%   22.79%   21.48%   21.35%   21.28%   20.63%
#                         report47  CAGR  25.52%   25.15%   24.03%   24.21%   24.03%   23.37%
#                         report53  CAGR  25.48%   25.11%   24.23%   24.27%   24.02%   23.36%
#                         report53  칼마  3.606    3.553    3.428    3.434    3.399    3.305
#                       **0.20(★)이 최적이었던 리포트는 40~53 중 한 번도 없다.** 0.10이 12/14에서
#                       칼마 정점이고, 나머지 2번도 0.15와 사실상 동률이며 둘 다 0.20을 크게 앞섰다.
#                       개선이 **한 셀이 아니라 0.10 > 0.15 > 0.20 순의 매끄러운 기울기**라는 점이
#                       중요하다 — 이 격자가 원래 판별하려던 '곡선맞춤'의 정반대 모습이다.
#
#                       리포트53 전 지표(0.10 vs 0.20 — 같은 격자 행이라 완전한 like-for-like):
#                         CAGR   25.48% vs 24.23%   (+1.25%p)
#                         샤프    2.192 vs 2.147
#                         칼마    3.606 vs 3.428    (+0.178 — 06c 전체 정점)
#                         MDD    -7.07% vs -7.07%  (**완전히 동일**)
#                         상승적중 +3.15 vs +3.08%p · 하락적중 +8.87 vs +8.00%p
#                         **놓친 큰상승일 55 vs 59일(4일 감소)** · 평균노출 0.5887 vs 0.5704
#                         거래 172 vs 188회(회전도 감소)
#                       여덟 지표가 **전부 같거나 좋다.** 사용자 채택 기준 ①(CAGR 무손실)
#                       ②(칼마 정점) ③(MDD 악화 ≤0.5%p)을 모두 통과하고, 06c 전체의 칼마 정점이다.
#
#                       [§A-2 왜 좋아지는가 — 인과가 기존 진단과 일치한다]
#                       매크로 복합점수는 **후행**한다. v1.10.0이 기록한 2020년 사례가 정확히 그것이다:
#                       코로나 급락에서 매크로 점수는 금리인하를 호재로 읽어 백분위 0.82~0.99로
#                       역주행했고, 실제 방어는 H트랙(규칙 ①②⑨)과 급락 트리거(⓪)가 했다.
#                       넓은 매크로 위험회피 밴드(하위 20%)는 **하락은 늦게 알리고 상승은 일찍 끊는다.**
#                       밴드를 하위 10%로 좁히면 늦고 틀린 매크로 위험회피만 걷히고 방어는 H트랙이
#                       그대로 한다 — **MDD가 -7.07%에서 한 자리도 움직이지 않은 것**이 그 증거다.
#                       (놓친 큰상승 4일 감소·하락적중 +0.87%p 동시 개선도 같은 이야기다.)
#
#                       [§B 또 격자의 끝이다 — 0.05·0.00을 심는다]
#                       0.10은 격자의 최하단이다. v1.40.0이 E5를 채택할 때 남긴 원칙(채택값 너머를
#                       반드시 재라)을 그대로 적용해 **0.05와 0.00**을 추가했다. 특히 0.00 행은
#                       "매크로 위험회피 밴드가 값을 하는가"라는 구조적 질문에 매 실행 답한다 —
#                       0.00이 이기면 규칙 하나를 지울 수 있다는 뜻이다.
#
#                       [§C ⚠ 죽은 파라미터 발견 — PCT_RISK_ON은 무효과다]
#                       격자의 on = 0.40/0.50/0.60/0.70 네 행이 **모든 off 수준에서 소수점까지
#                       동일**하다. 이유는 구조다: 규칙 ⑥(추세승격)이 점수 ≥0.30 & 가격>200MA &
#                       H<0.70이면 어차피 RISK_ON으로 올리고, 규칙 ②가 H≥0.70이면 어차피 중립으로
#                       막는다. 즉 RISK_ON 임계값이 닿는 날이 남아 있지 않다.
#                       → 24조합을 **14조합으로 줄이고**(off 8 × on 2) on은 canary로 2행만 남겼다.
#                       계산은 42% 줄고, 다시 달라지면 규칙 스택이 바뀐 것이므로 테스트가 잡는다.
#
#                       [§D ⚠ 버그 수정 — 06c DEEP_RECOVERY_DD 격자는 like-for-like가 아니었다]
#                       그 격자만 `generate_signals(...)`에 **`struct_dd=struct_dd`를 넘기지 않아**
#                       규칙 ⑧(구조적 저점 확인 승격)이 꺼진 채로 재시뮬레이션됐다. 라이브와 같아야
#                       할 ★ 행(-15%)이 리포트 47~53 **전부에서** CAGR 23.42% / 칼마 3.314로 찍혔다
#                       (라이브 24.23% / 3.428). 파일 안 22개 호출부 중 이 한 곳만 빠졌다.
#                       v0.24.0에서 sector_rotation.py의 [상한격자]가 오버라이드를 못 받아 "상한
#                       0.8이 최적"이라는 틀린 결론을 냈던 것과 **같은 버그 유형**이다.
#                       → `struct_dd=struct_dd` 추가. 이 격자로 내렸던 -12/-15/-20% 비교는 모든 행이
#                       같은 결손을 공유했으므로 상대 순서는 유효하나 절대 수준은 다음 리포트부터
#                       라이브와 맞는다. 회귀 테스트가 "모든 06c 호출부의 인자 집합이 같은가"를
#                       매 실행 확인한다.
#
#                       [§E 채택하지 않은 것 — 과열 헤어컷 E7]
#                       E7((0.10,0.6),(0.12,0.4))은 리포트 47~53 **여덟 번 연속** E5를 CAGR
#                       +0.12~0.13%p · 샤프 +0.05 · 칼마 +0.017로 앞섰고 MDD는 동일했다. 기준
#                       ①②③을 통과하는 후보지만 **이번 라운드에는 채택하지 않는다** — 활성 신호
#                       파라미터를 한 커밋에 둘 이상 바꾸지 않는다(v1.4.0 §9). PCT_RISK_OFF가
#                       바뀌면 이격도가 걸리는 국면 구성도 바뀌므로 E7의 우위는 **새 라이브 위에서
#                       다시 측정돼야** 한다. E9 행을 추가해 두 칸이 항상 측정되게 했다.
#                       같은 이유로 TREND_PROMOTION_MIN_SCORE_PCT도 0.30을 유지하되, 새 경계의
#                       등가값(0.10 + 버퍼 0.10 = 0.20)을 격자에 추가만 했다.
#
#                       [§F 다음 리포트에서 반드시 확인할 것]
#                       ① 09_국면통계의 국면정의 검증(위험선호 익일평균 > 위험회피 익일평균)이
#                          좁아진 밴드에서도 PASS인가 — FAIL이면 즉시 0.20으로 되돌린다.
#                       ② 0.05·0.00 행이 0.10보다 나쁜가(나쁘면 0.10이 진짜 정점).
#                       ③ DEEP 격자 ★ 행이 라이브와 일치하는가(§D 수정 확인).
#                       ④ E7·추세승격 0.20이 새 라이브 위에서도 여전히 앞서는가.
#
#  VERSION: v1.43.0 - 2026-09-10 - [⚠ 사전등록 격자 판정: ⑤⑦⑧ 제외 **기각** + 범위를 좁힌 '⑦⑧만'
#                       행 추가] 변경 모듈: `Config.NEUTRAL_RISK_CUT_EXEMPT_SCOPE`(신규, "all"),
#                       규칙 ⑨ 블록, 06c NEUTRAL_RISK_CUT 격자(7행 → **8행**), 로그 1필드.
#                       **신호 로직·채택값 무변경 — v1.42.0과 비트 동일**(제외 자체가 여전히 off).
#
#                       [§1 v1.42.0 검증 완료 — 감사 구멍이 실제로 메워졌다]
#                       리포트50에서 `폭조절(B)` 컬럼이 정상 출력됐고(발동 145일), **'확정 중립인데
#                       목표비중 0이고 아무 규칙도 발동 안 한 날'이 397일 중 0일**이 됐다
#                       (v1.42.0 이전 75일 → 그 중 76일이 폭조절(B)로 설명됨. 76은 데이터 개정으로
#                       하루 늘어난 수치다). 예고한 검증 지점을 정확히 통과했다.
#
#                       [§2 ⚠ 사전등록 격자 판정 — 기각. 재보길 잘했다]
#                       리포트50 06c(실제 엔진):
#                         0.0@H>0.70 (★)      CAGR 24.23% 샤프 2.147 상승 3.08 하락 8.00 놓친 59 MDD -7.07% 칼마 3.428
#                         0.0@H>0.70 ⑤⑦⑧제외   CAGR 23.84% 샤프 2.057 상승 2.14 하락 8.85 놓친 **47** MDD -7.45% 칼마 3.201
#                         0.25@H>0.70 ⑤⑦⑧제외  CAGR 23.11% 샤프 1.996 상승 2.14 하락 8.85 놓친 47 MDD **-11.11%** 칼마 2.080
#                       CAGR -0.39pp · 샤프 -0.09 · 칼마 -0.227 · MDD 0.38%p 악화 · 큰하락일 노출
#                       0.3855 -> **0.4355**. → **기각.** 지난 라운드 리플레이가 "충돌일 익일평균
#                       -0.031%"로 예고한 방향과 실제 엔진이 일치했다 — 리플레이만 보고 채택했다면
#                       네 지표가 동시에 나빠졌을 것이다. 두 행은 **기각 근거로 격자에 남긴다.**
#
#                       [§3 그래도 남은 신호 — 놓친 큰상승을 12일 줄인 유일한 변형이었다]
#                       기각 행은 06c 전체에서 **놓친 큰상승을 59 -> 47일로 줄인 유일한 변형**이고
#                       하락 적중도 8.85로 격자 최고였다(NEUTRAL_RISK_CUT off는 43일까지 줄지만
#                       MDD가 -14.66%로 무너진다). 대가는 꼬리(큰하락일 노출·MDD)에 몰려 있다.
#                       가설: 대가는 **확인 조건이 약한 ⑤**(120일 저점 +2% 반등, 추세 확인 없음)에서
#                       나오고, ⑦(-15% 낙폭 후 회복)·⑧(가격>200MA & H<0.50)은 이미 강한 확인을
#                       요구하므로 꼬리를 덜 상한다. → 범위를 좁힌 **'⑦⑧만 제외(⑤ 유지)'** 행을
#                       하나 더 심었다(같은 격자의 확장이지 새 탐색이 아니다). 라이브는 무변경.
#
#                       [§4 다른 격자는 전부 현행 유지가 최적]
#                       회복확인폭 0.015 = 0.02(★) 동률(포화) · HAZARD_ENTER 0.90(★) CAGR·칼마·
#                       놓친·하락적중 모두 최적 · FAST_TRIGGER 0.97(★) 최적 · 추세승격 0.30(★) 최적.
#                       ⚠ 과열해어컷 E7이 이번에도 CAGR 24.36%/샤프 2.199/칼마 3.446으로 E5(★)를
#                       앞섰지만, **하락 적중 8.00 -> 6.81 · 놓친 큰상승 59 -> 61**로 세 번째 리포트
#                       연속 같은 패턴이다 — 사용자 기준("크게 상승했는데 회피하면 안 된다")에
#                       어긋나므로 이번에도 채택하지 않는다.
#
#                       [§5 실측 차이의 출처 — 데이터 개정이지 코드가 아니다]
#                       리포트49(v1.41.1) 24.02% -> 리포트50(v1.42.0) 24.23%. v1.42.0은 표시·격자만
#                       바꿨으므로 신호가 달라질 수 없다. 두 리포트를 일자별로 대조하니 복합점수
#                       백분위가 1,964일(최대 0.0285), H 백분위가 1,911일(최대 0.1343) 다르다 —
#                       FRED 매크로 시리즈 개정으로 워크포워드 재추정 결과가 미세하게 이동한 것이고,
#                       그 결과 **목표비중이 달라진 날은 2,184일 중 5일**뿐이다. 코드 변경의 결과가
#                       아님을 기록해 둔다(재현성 판단 기준).
...
#
#                       [§1 v1.40.0 실측 — 격자와 소수점까지 일치]
#                       리포트48: CAGR **23.83%** · 샤프 **2.119** · MDD **-7.07%** · 칼마 **3.372**.
#                       06c의 E5 행(23.83 / 2.119 / -7.07 / 3.372)과 **완전히 같다.**
#                       다섯 라운드 연속으로 격자가 옳았다.
#
#                       [§2 ⚠ E7 기각 — 경계 확장이 정확히 제 역할을 했다]
#                       v1.40.0에서 "E5가 격자 끝이라 너머가 미측정"이라며 E7을 넣었다. 결과:
#                         E5  CAGR 23.83%  샤프 2.119  칼마 3.372  상승 +2.99  하락 +7.68  놓친 60
#                         E7  CAGR 23.96%  샤프 2.170  칼마 3.390  상승 +4.41  하락 **+6.60**  놓친 **62**
#                       수익 지표는 계속 좋아지지만 **하락 적중이 1.08%p 나빠지고 놓친 큰상승이 2일
#                       늘어난다.** E5가 깨끗했던 이유는 '예측 3지표가 전부 불변'이었기 때문인데
#                       E7은 그 성질을 잃는다. 사용자 기준("크게 상승했는데 회피하면 안된다")에도
#                       어긋난다 → **기각.** 경계를 재보지 않았다면 E7이 더 좋아 보였을 것이다.
#
#                       [§3 채택 — 회복확인폭 0.02. 세 라운드를 기다린 값이다]
#                       v1.38.0에서 MDD -8.42%로 기각, v1.40.0에서 "한 라운드 한 파라미터"로 보류.
#                       기준선이 bear_only+HAZARD0.90+E5로 바뀌며 MDD가 -7.07%로 내려앉았고
#                       **정확도가 세 갈래 모두 개선된다**: 상승 +2.99→+3.05 · 하락 +7.68→+7.95 ·
#                       놓친 큰상승 60→59 · 큰하락일 노출 0.3855 완전 동일.
#                       CAGR 23.83→24.24% · 샤프 2.119→2.147 · 칼마 3.372→3.429 · MDD 불변.
#                       ⚠ 0.02도 격자 끝이므로 **0.015를 추가**해 다음 실행이 경계를 재게 했다.
#
#                       [§4 ⚠ 섹터와 동시 변경 — 커플링을 명시한다]
#                       이번 라운드는 S v0.25.0(하락국면리더 1.0 → 0.5)과 함께 나간다. 두 층은 E_t로
#                       연결되므로 엄밀히는 조합이 미측정이다. 다만 방향이 **같다** — 회복확인폭을
#                       풀면 규칙 ⑤가 더 자주 발동해 E_t=0 구간이 줄고, 그러면 하락국면리더 발동일도
#                       줄어든다. 서로 상쇄가 아니라 보강이다. 섹터 쪽 효과(칼마 2.144 → 2.909)는
#                       M의 +0.41pp CAGR 변화가 뒤집을 수 있는 크기가 아니다. 두 격자 모두 상설이라
#                       다음 실행이 새 기준선에서 다시 잰다.
#
#  VERSION: v1.40.0 - 2026-09-09 - [⚠ 과열 해어컷 E3 → E5 — 정확도 열이 '예측 무변경 + 하락노출만 감소'를
#                       보여준 가장 깨끗한 채택 + 06c 경계 확장(E7)]
#                       변경 모듈: `Config.EXTENSION_HAIRCUT_STEPS`, 06c 해어컷 격자(+E7). 신호 로직 무변경.
#
#                       [§1 v1.39.0 실측 — 또 격자 예측을 넘었다]
#                       리포트47: CAGR **23.69%** · 샤프 **2.062** · MDD **-7.07%** · 칼마 **3.351**.
#                       06c 예측(23.35 / 2.039 / 3.303)보다 좋다. 네 라운드 연속으로 격자가 옳았다.
#
#                       [§2 채택 — 이보다 깨끗한 모양은 없다]
#                       E3 → E5에서 **상승 적중 +2.99%p, 하락 적중 +7.68%p, 놓친 큰상승 60일이
#                       셋 다 완전히 동일**하고, **큰하락일 평균노출만 0.3991 → 0.3855로 줄어든다.**
#                       예측을 하나도 바꾸지 않고 하락 노출만 깎는다 — 샤프 2.062 → **2.119**(06c 최고),
#                       칼마 3.351 → 3.372, MDD -7.07% 불변.
#                       근거는 단조성이다: off 2.629 < E1 3.210 < E3 3.351 < E5 3.372.
#                       ⚠ E5가 격자의 끝이라 그 너머는 미측정이다. 임의로 더 낮추지 말고
#                       **E7((0.10,0.6),(0.12,0.4))을 격자에 추가**했으니 다음 실행이 답한다.
#
#                       [§3 채택하지 않은 것 — RECOVERY_CONFIRM_PCT 0.02]
#                       새 기준선에서 0.02가 CAGR 24.09% · 샤프 2.090 · 칼마 3.408 · 놓친 59일로
#                       현행 0.03보다 낫다(두 라운드 전에는 MDD가 -8.42%로 터져 기각했었는데,
#                       기준선이 bear_only+HAZARD 0.90으로 바뀌며 -7.07%가 됐다).
#                       ⚠ 그러나 **한 라운드에 한 파라미터**가 원칙이다 — E5와 0.02를 함께 바꾸면
#                       그 조합은 아무도 측정하지 않은 상태가 된다(v1.34.0의 실패 원인).
#                       0.02는 격자에 그대로 남아 다음 실행이 새 기준선에서 다시 잰다.
#
#  VERSION: v1.39.0 - 2026-09-09 - [⚠ HAZARD_ENTER 0.85 → 0.90 — v1.38.0에 신설한 06c 정확도 열이
#                       처음으로 판정 근거가 됐다 + 섹터 진단(사용자 지적 "국면 대비 섹터 수익률이 안 나온다")]
#                       변경 모듈: `Config.HAZARD_ENTER`(0.85→**0.90**). 신호 로직 무변경.
#
#                       [§1 v1.38.0 실측 — 격자 예측과 소수점 셋째 자리까지 일치했다]
#                       리포트46: CAGR **22.49%** · 샤프 **2.027** · MDD **-7.07%** · 칼마 **3.182**.
#                       06c가 예측한 bear_only 행(22.49 / 2.027 / -7.07 / 3.182)과 **완전히 같다.**
#                       "실제 엔진이 잰 값만 채택한다"는 원칙이 세 라운드 연속으로 맞았다.
#                       일간승률도 0.6832 → **0.6969**로 올랐다.
#
#                       [§2 채택 — 정확도 열이 없었으면 못 골랐을 판정]
#                       HAZARD_ENTER 0.90은 CAGR +0.86pp · 칼마 +0.121인데, 종전 방식(CAGR·샤프만)
#                       으로는 규칙 ⑮ both와 구분이 안 됐다. 둘 다 "CAGR이 오르는 대신 큰하락일
#                       노출도 오르는" 모양이기 때문이다. 정확도 열이 그 둘을 갈랐다:
#                         규칙 ⑮ both : 큰하락 노출 ↑ + **MDD -7.07 → -10.48%**   → 기각(v1.36.0)
#                         HAZARD 0.90 : 큰하락 노출 ↑ + **MDD -7.07% 불변**        → 채택
#                       그리고 0.90은 **놓친 큰상승을 63 → 60일로 3일 회수**한다(큰상승일 노출
#                       0.3800 → 0.4073). 사용자 기준("크게 상승했는데 회피하면 안된다")에 직접 부합한다.
#                       ⚠ 0.90과 0.95가 완전히 동일 = 평탄지대이지 칼날 끝이 아니다.
#
#                       [§3 기각 — TREND_PROMOTION 0.35]
#                       새 기준선에서도 CAGR 22.33%(-0.16pp) · 칼마 3.159(-0.023)로 현행보다 낮다.
#                       하락 적중(+7.63%p)과 큰하락일 노출(0.3655)은 가장 좋지만 칼마가 내려간다.
#                       세 라운드 연속으로 같은 결론이다 — 더 시도하지 말 것.
#
...
#                       변경 모듈: `Config.RECOVERY_CONFIRM_PCT`(0.05→**0.03**),
#                       `threshold_sensitivity()` 06c 회복확인폭 격자(4→6수준). 신호 로직 무변경.
#
#                       [§1 진단 — 놓친 큰상승 68일이 어디 있는지 먼저 확정했다]
#                       리포트44(v1.36.0 실행) 기준 큰상승(상위5%) 110일 중 68일을 비중 0.5 미만으로
#                       보냈다. 그 68일의 소재: **하락상태 55일 · 중립 13일.** 연도별로는 2022년이
#                       33일 중 31일(93.9%)로 압도적이고 2020년 15/28, 2018년 6/8이다.
#                       → 문제는 중립이 아니라 **하락상태(=고변동 국면)의 꼬리**에 있다.
#                       상태별 꼬리 구성(그날 실현):
#                         상승 1260일: 큰상승 32 · 큰하락 36 (꼬리비 0.89) · 노출 0.868
#                         중립  532일: 큰상승 19 · 큰하락 19 (꼬리비 1.00) · 노출 0.342
#                         하락  389일: 큰상승 **59** · 큰하락 55 (꼬리비 **1.07**) · 노출 0.043
#                       평균은 정확히 정렬되는데(+10.86 / +4.13 / -6.16bp) **꼬리는 역정렬**이다.
#                       하락상태 자체는 유지해야 한다 — 그 구간을 현금으로 보낸 순효과가 +30.84%다.
#
#                       [§2 이번에 기각한 것 — 14건. 다시 시도하지 말 것]
#                       (a) 하락상태 내부 분리자 5종(종가>MA20 / >MA50 / 위험점수 5일하락 / 복합점수
#                           5일상승 / 120일저점+5%): 4종이 역부호이고, 유일한 양수 ⓒ는 연도별
#                           -25.6/+67.7/+42.9/**-223.2**/+16.2로 3/5 — 기각.
#                           ⚠ 부수 소득: ⓐ(규칙 ⑭에서 -20% 낙폭 게이트만 제거)가 -13.21bp로
#                           크게 나빠진다 = **규칙 ⑭의 낙폭 게이트가 하중을 받고 있다는 증거**다.
#                       (b) 상승상태 내부 눌림목 4종(종가<MA20 / <MA50 / 고변동 / 낙폭<=-5%):
#                           전부 익일수익이 **더 높다**(+13~+22bp). 그런데 그 날들의 목표비중이
#                           이미 0.936~1.000이다 — **시스템이 이미 만점을 싣고 있어 먹을 것이 없다.**
#                       (c) 휩쏘(전환 직후 되돌림): 하락 스펠 14개·중앙값 20일·**1~2일짜리 0개**,
#                           놓친 큰상승 59일 중 46일이 '11일 이상' 구간 — 휩쏘 문제가 아니다.
#                       (d) 노출을 깎는 규칙 6종 감사(④⑨⑩⑫·속도경보·위험선호차단): **전부 옳게
#                           발동**한다(발동일 익일수익이 비발동일보다 낮음). 고칠 것이 없다.
#                       (e) ⑥ 추세승격 임계 0.30 → 0.35: 06c 샤프가 1.827/1.838/**1.856**/1.832로
#                           내부 정점이고 CAGR은 단조 감소다. 비단조 정점 + 크기 +0.018 = 잡음 —
#                           기각. (감사에서 ⑥은 익일 +4.96bp(0.93σ)로 시장평균 +6.20bp에도 못 미치는
#                           약한 규칙임이 드러났지만, 조인다고 나아지지 않는다는 것이 실측 결론이다.)
#
#                       [§3 채택 — 노출을 '더하는' 규칙 감사가 답을 줬다]
#                       깎는 규칙만 감사했지 **더하는 규칙은 한 번도 감사하지 않았다.** 이번에 했다:
#                         ⑤ 회복승격     185일 · 익일 **+18.99bp** (+2.15σ) · 목표비중 0.173  ← 우량
#                         ⑦ 깊은낙폭회복  88일 · 익일 **+29.48bp** (+2.30σ)                  ← 우량
#                         추세오버라이드  324일 · 익일 +13.48bp (+2.02σ)                     ← 우량
#                         ⑥ 추세승격     510일 · 익일  +4.96bp (0.93σ) · 목표비중 0.835      ← 약함
#                         위험회피해제안전판 176일 · 익일 +0.41bp (0.05σ) · 목표비중 0.605    ← 무위험 이하
#                       **⑤가 가장 우량한데 가장 적게 발동한다(185일).** 그래서 새 규칙을 만들지 않고
#                       **⑤의 확인폭을 푼다.** 근거·수치·정직 고지는 RECOVERY_CONFIRM_PCT 주석에 상세.
#                       요약: 06c 실제 엔진에서 CAGR 19.88→**20.97%** · 샤프 1.838→**1.898** ·
#                       칼마 2.813→**2.925** · MDD -7.07→-7.17%(-0.10pp). 네 수준 전부 단조.
#                       정확도: 큰상승 포착 0.574→**0.634** · 놓친 큰상승 **68→63일** ·
#                       하락 적중 기저대비 +7.26→**+7.50%p** · 큰하락 회피 0.355→0.357(악화 없음).
#                       ⚠ 표본 28일로 얇다 — 주석의 정직 고지 참조.
#
...
#                       실제와 3%p 달랐음). T+1 시가 체결·현금레그까지 반영해 리포트 원본 MDD를 정확히 재현하는
#                       리플레이로 다시 측정한 결과: (1) M은 이미 추세가 꺾일수록 노출을 계단식으로 줄이고 있다
#                       (평균 목표비중 종가>MA20 0.657 → <MA20 0.376 → <MA50 0.302 → <MA200 0.191 → 60일고점
#                       -10% 이하 0.153), (2) 표준 기술적 꺾임 신호 8종(MA20 이탈·MA50 기울기·60일고점 낙폭·MACD·
#                       RSI·샹들리에·20일 신저가·사다리)을 얹으면 CAGR가 12.9~17.9%로 전부 떨어지고 MDD는 하나도
#                       개선되지 않는다, (3) 변동성 타깃도 MDD -7.07→-6.88%(0.19%p)에 CAGR -1.5~-4.6%p로 열위.
#                       실제 MDD는 2020-06-08~11 4거래일 단일 충격(SPY -6.99%, 국면 '상승'·비중 1.0)이라 사전 감축
#                       규칙으로는 닿지 않는다. 계산·신호는 v1.25.0과 비트 동일로 복귀.
...
class Config:
    ...
    # [v1.58.0 R88 ⚠⚠ 위험 파라미터] 0.50 → **0.60** — 사용자 신뢰도 '참여' 손잡이(사용자 선택: 양쪽형). 근거·되돌리기는 머리 CHANGELOG v1.58.0.
    #   되돌리기: m_overrides={"POS_NEUTRAL": 0.5}
    POS_NEUTRAL: float = 0.60
    ...
    # [v1.44.0 §A ⚠⚠ 신호(위험) 파라미터 — 이번 라운드 유일한 활성 변경] PCT_RISK_OFF 0.20 → 0.10.
    #  06c 국면 임계값 격자는 원래 "특정 값에서만 좋으면 곡선맞춤"을 보려고 만든 **점검용**이라
    #  채택 후보로 읽지 않았다. 그런데 리포트 40~53 **열네 번**을 모아 보니 그 격자는 점검이
    #  아니라 **답을 계속 내고 있었다**:
    #    위험회피 백분위   0.10     0.15     0.20(★)  0.25     0.30     0.35
    #    report47  CAGR  25.52%   25.15%   24.03%   24.21%   24.03%   23.37%
    #    report53  CAGR  25.48%   25.11%   24.23%   24.27%   24.02%   23.36%
    #    report53  칼마  3.606    3.553    3.428    3.434    3.399    3.305
    #  **0.20(★)이 최적이었던 리포트는 40~53 중 단 한 번도 없다.** 0.10은 12/14 리포트에서
    #  칼마 정점이고 나머지 2번도 0.15와 사실상 동률(0.10과 0.15 차 ≤0.003)이며, 두 값 모두
    #  0.20을 큰 폭으로 앞섰다. 개선은 **한 셀이 아니라 0.10 > 0.15 > 0.20 순의 매끄러운 기울기**라
    #  곡선맞춤의 반대 모습이다(이 격자의 원래 목적이 바로 그 판별이었다).
    #  ⚠ 리포트53 전 지표 비교(0.10 vs 0.20 — like-for-like, 나머지 파라미터 동일):
    #    CAGR 25.48 vs 24.23% (+1.25%p) · 샤프 2.192 vs 2.147 · 칼마 3.606 vs 3.428 (+0.178)
    #    **MDD -7.07%로 동일** · 상승적중 +3.15 vs +3.08%p · 하락적중 +8.87 vs +8.00%p
    #    **놓친 큰상승일 55 vs 59일(4일 감소)** · 평균노출 0.5887 vs 0.5704 · 거래 172 vs 188회
    #  즉 **여덟 지표가 전부 같거나 좋다.** 사용자 채택 기준 ①(CAGR 무손실) ②(칼마 정점)
    #  ③(MDD 악화 ≤0.5%p)을 전부 통과하며, 06c 전체에서 칼마 정점이다.
    #  [왜 좋아지는가 — 인과] 매크로 복합점수는 **후행**한다(v1.10.0이 기록한 2020년 사례:
    #  금리인하를 호재로 읽어 백분위가 0.82~0.99로 역주행). 넓은 매크로 위험회피 밴드(하위 20%)는
    #  하락을 늦게 알리면서 상승은 일찍 끊는다. 실제 방어는 위험(H)트랙 — 규칙 ①(H>0.90 강제진입),
    #  ②(H>0.70 승격차단), ⑨(중립 & H>0.70 → 0), ⓪(급락 트리거) — 이 이미 하고 있다.
    #  밴드를 하위 10%로 좁히면 **늦고 틀린 매크로 위험회피만 걷히고 방어는 H트랙이 그대로 한다.**
    #  MDD가 한 자리도 안 움직인 것(-7.07% 동일)이 그 증거다.
    #  ⚠ 결합 효과(숨기지 않는다): PCT_RISK_OFF는 DEEP_RECOVERY_SCORE_GUARD(규칙 ⑦의 점수 가드)와
    #  TREND_PROMOTION_MIN_SCORE_PCT(0.30 = 0.20 + 버퍼 0.10)의 의미론에도 쓰인다. 가드는 함께
    #  느슨해지고, 추세승격 버퍼는 0.10 → 0.20으로 넓어진 셈이 된다. **둘 다 이번에 손대지
    #  않는다**(한 라운드 한 파라미터). 대신 06c 추세승격 격자에 0.20('0.10+버퍼 0.10' 등가)을
    #  추가해 다음 실행이 재게 한다.
    #  ⚠ 검증 지점(다음 리포트에서 확인할 것): 09_국면통계의 국면정의 검증(위험선호 익일평균 >
    #  위험회피 익일평균)이 좁아진 밴드에서도 PASS인지, 그리고 0.05/0.00 행이 0.10보다 나쁜지.
    #  되돌리려면 0.20.
    PCT_RISK_ON: float = 0.60          # 백분위 > 이 값 -> 위험선호(100%) [v1.4.0: 0.50→0.60]
                                       # ⚠ [v1.44.0 §C] 이 값은 **현재 규칙 스택에서 사실상 무효과**다 —
                                       #   06c 격자의 0.40/0.50/0.60/0.70 네 행이 모든 off 수준에서
                                       #   소수점까지 동일하다. 규칙 ⑥(추세승격)이 점수 ≥0.30 & 가격>200MA &
                                       #   H<0.70이면 어차피 RISK_ON으로 올리고, 규칙 ②가 H≥0.70이면
                                       #   어차피 중립으로 막기 때문이다. 격자에 canary로 2행만 남긴다.
    PCT_RISK_OFF: float = 0.10         # 백분위 < 이 값 -> 위험회피(0%) [v1.44.0: 0.20→0.10]
    ...
    # [v1.39.0 ⚠ 0.85 → 0.90 — v1.38.0에서 신설한 06c '정확도 열'이 처음으로 판정에 쓰였다]
    #  리포트46 06c(실제 엔진, HAZARD_BLOCK=0.70 고정):
    #     0.75  CAGR 21.24%  샤프 1.942  MDD -7.07%  칼마 3.006  놓친 큰상승 65일
    #     0.80  CAGR 21.04%  샤프 1.914  MDD -7.07%  칼마 2.977  놓친 큰상승 64일
    #     0.85  CAGR 22.49%  샤프 2.027  MDD -7.07%  칼마 3.182  놓친 큰상승 63일   ← 종전
    #     0.90  CAGR 23.35%  샤프 2.039  MDD -7.07%  칼마 3.303  놓친 큰상승 60일   ← 채택
    #     0.95  CAGR 23.35%  샤프 2.039  MDD -7.07%  칼마 3.303  놓친 큰상승 60일   (0.90과 완전 동일)
    #  ⚠ 0.90과 0.95가 완전히 같다 = 이 구간에서 제약이 걸리지 않는다(평탄지대). 칼날 끝이 아니다.
    #  [정확도 열이 말하는 것 — 이 판정은 v1.38.0 이전에는 불가능했다]
    #     큰상승일 평균노출  0.3800 → **0.4073**  · 놓친 큰상승 63일 → **60일**(3일 회수)
    #     큰하락일 평균노출  0.3809 → 0.3991     · **MDD는 -7.07%로 불변**
    #     상승 적중 기저대비 +2.96 → +2.88%p · 하락 적중 +7.42 → +7.37%p (둘 다 잡음 범위)
    #  ⚠ 정직하게: 큰하락일 노출도 0.018 올라간다 — v1.36.0에서 규칙 ⑮ both를 기각한 '양쪽 다
    #  올리기' 모양이다. 그때와 다른 점은 (i) **MDD가 전혀 나빠지지 않고**(-7.07% 불변),
    #  (ii) 칼마가 3.182 → 3.303으로 오르며, (iii) 적중률이 사실상 그대로라는 것이다.
    #  규칙 ⑮ both는 MDD를 -7.07 → -10.48%로 터뜨렸다. 그 차이가 채택/기각을 가른다.
    #  되돌리려면 0.85. 06c에 5수준이 정확도 열과 함께 그대로 남아 있다.
    HAZARD_ENTER: float = 0.90         # H 백분위가 이 값을 넘으면(위험 급등) 단독으로 위험회피 진입
    ...
    # [v1.37.0 ⚠ 0.05 → 0.03 — 위 v1.11.0 주석이 "다음 실행부터 06c로 확인한다"고 사전등록한
    #  민감도가 리포트44에서 답을 냈다. 사후에 만든 격자가 아니라 6버전 전에 미리 박아둔 격자다.]
    #  리포트44 06c(실제 엔진, 같은 격자 4수준) — 네 지표가 전부 단조다:
    #     0.03  CAGR 20.97%  샤프 1.898  MDD -7.17%  칼마 2.925  발동 244일   ← 채택
    #     0.05  CAGR 19.88%  샤프 1.838  MDD -7.07%  칼마 2.813  발동 189일   (종전)
    #     0.07  CAGR 19.67%  샤프 1.835  MDD -7.07%  칼마 2.784  발동 178일
    #     0.10  CAGR 18.49%  샤프 1.781  MDD -7.07%  칼마 2.617  발동 133일
    #  ⚠ 단조성이 핵심 근거다. 조이면 단조로 나빠지고 풀면 단조로 좋아진다 — 한 점이 우연히
    #  좋은 것이 아니라 현행 0.05가 기울기 위에 얹혀 있었다는 뜻이다.
    #  [정확도 — 규칙 ⑮가 실패했던 바로 그 검정을 통과한다] 리포트44 목표비중 기준:
    #     큰상승 포착   0.574 → 0.634   · 놓친 큰상승 68일 → 63일 (5일 회수)
    #     하락 적중 기저대비 +7.26 → +7.50%p
    #     큰하락 회피   0.355 → 0.357 (악화 없음)
    #     상승 적중 기저대비 +2.58 → +2.56%p (변화 없음, 잡음 범위)
    #  규칙 ⑮는 적중률 불변·큰하락회피 악화·수익률만 상승이라 기각했다. 이번은 반대다 —
    #  놓친 상승을 실제로 회수하면서 하락 방어를 건드리지 않는다. 사용자 기준을 그대로 만족한다.
    #  ⚠ 정직 고지 — 표본이 얇다: 0.05 → 0.03에서 목표비중이 달라지는 날은 **28일뿐**이고
    #  (2018 4일 · 2019 2일 · 2022 19일 · 2026 3일) 이득이 2019-01-03~04(2일)와 2022에 몰려 있다.
    #  다만 (i) 어느 해에도 마이너스가 아니고(4개년 +, 5개년 0), (ii) 그 28일의 익일수익 평균은
    #  +59.40bp로 전체표본 +6.15bp의 10배이며(+2.6σ), (iii) 규칙 ⑤ 자체가 승격일 익일 +18.99bp
    #  (+2.15σ)로 이미 검증된 '좋은' 규칙이다 — 새 규칙을 만든 게 아니라 검증된 규칙을 덜 조인 것이다.
    #  ⚠ 0.02/0.04는 리플레이로만 봤고(실제 엔진 미측정) 리플레이는 06c 대비 CAGR을 과대평가한다
    #  (+1.76pp vs 실제 +1.09pp — v1.34.0에서 같은 함정에 빠졌다). 그래서 **실제 엔진이 측정한
    #  0.03만 채택**하고 0.02/0.04는 06c 격자에 넣어 다음 실행이 실측하게 한다.
    # [v1.41.0 ⚠ 0.03 → 0.02 — 세 라운드를 기다린 값이 드디어 전 지표에서 우세해졌다]
    #  이 값은 v1.38.0 때 0.02가 MDD -8.42%로 터져 기각했고, v1.40.0 때는 "한 라운드 한 파라미터"
    #  원칙 때문에 보류했다. 리포트48(실제 엔진)에서 기준선이 bear_only+HAZARD0.90+E5로 바뀌자
    #  MDD가 -7.07%로 내려앉았고, **정확도 3지표가 전부 개선**된다:
    #     0.02  CAGR 24.24%  샤프 2.147  MDD -7.07%  칼마 3.429  상승 +3.05  하락 +7.95  놓친 59
    #     0.03  CAGR 23.83%  샤프 2.119  MDD -7.07%  칼마 3.372  상승 +2.99  하락 +7.68  놓친 60  ← 종전
    #     0.04  CAGR 23.65%  샤프 2.106  MDD -7.07%  칼마 3.346  상승 +3.02  하락 +7.86  놓친 60
    #     0.05  CAGR 22.21%  샤프 2.018  MDD -7.07%  칼마 3.143  상승 +2.84  하락 +7.36  놓친 64
    #  **상승 적중 +2.99→+3.05 · 하락 적중 +7.68→+7.95 · 놓친 큰상승 60→59 · 큰하락일 노출
    #  0.3855로 완전 동일.** 정확도가 세 갈래 모두 좋아지면서 하락 노출은 그대로다.
    #  칼마도 단조(0.02 3.429 > 0.03 3.372 > 0.04 3.346 > 0.05 3.143 > 0.07 3.109 > 0.10 2.881).
    #  ⚠ 0.02가 격자의 끝이라 그 너머는 미측정이다 — E5 채택 때 E7을 넣어 경계를 확인했던 것과
    #  같은 방식으로 **0.015를 격자에 추가**했다. 임의로 더 낮추지 말 것.
    RECOVERY_CONFIRM_PCT: float = 0.02
    ...
    # [v1.43.0 §B] 제외 범위. "all"=⑤⑦⑧ 전부, "deep"=⑦⑧만(깊은 낙폭·구조적 저점).
    #   리포트50이 "all"을 기각했지만(아래 §A) 그 행은 **놓친 큰상승을 59 → 47일로 줄인 유일한
    #   변형**이었다 — 대가는 큰하락일 노출 0.3855 → 0.4355와 MDD -7.07 → -7.45%였다.
    #   가설: 대가는 확인 조건이 약한 ⑤(120일 저점 +2% 반등, 추세 확인 없음)에서 나오고,
    #   ⑦(-15% 낙폭 후 회복)·⑧(가격>200MA & H<0.50)은 이미 강한 확인을 요구하므로 꼬리를 덜 상한다.
    #   그래서 범위를 좁힌 "deep" 행을 06c에 하나 더 심어 다음 실행이 판정하게 한다(라이브 무변경).
    NEUTRAL_RISK_CUT_EXEMPT_SCOPE: str = "all"
    ...
    #   ⚠ [v1.27.0 기본 비활성으로 되돌림 — 실측 반증] 사용자 실행(리포트35, v1.26.0)에서 CAGR 18.65% → 17.96%
    #   (-0.69%p)이고 MDD는 -7.07%로 **전혀 개선되지 않았다**. v1.26.0에서 인용한 'MDD -10.07→-7.39%'는 종가기준
    #   근사 리플레이의 산물이었고(그 리플레이의 기준 MDD -10.07% 자체가 실제 -7.07%와 달랐다), T+1 시가 체결과
    #   현금레그를 반영한 충실 리플레이(리포트 원본 MDD를 소수점까지 재현)로 다시 재면 개선 여지가 없다.
    #   근본 원인: M의 실제 최대낙폭은 2020-06-08~06-11의 **4거래일 단일 충격**(SPY -6.99%, 그 4일 내내 국면이
    #   '상승(위험선호)'이고 목표비중 1.0)이라, 추세가 멀쩡한 상태에서 터진 사건이다. 노출을 미리 줄이는 어떤
    #   규칙도 이 낙폭을 건드리지 못하고 상승기 수익만 깎는다. 코드와 근거는 남겨두되 기본은 끈다.
    USE_CRASH_PRETRIGGER_CUT: bool = False   # [v1.27.0] 기본 끔 — 실측에서 CAGR만 깎고 MDD 개선 0(아래 근거)
    ...
    # [v1.40.0 ⚠ E3((0.10,0.8),(0.12,0.6)) → E5((0.10,0.7),(0.12,0.5))]
    #  리포트47 06c(실제 엔진) — 해어컷이 강해질수록 칼마가 **단조 증가**한다:
    #     off(상한 없음)  CAGR 23.00%  샤프 1.856  MDD -8.75%  칼마 2.629
    #     E1 ≥10%→0.9    CAGR 23.35%  샤프 1.962  MDD -7.27%  칼마 3.210
    #     E3 ≥10%→0.8    CAGR 23.69%  샤프 2.062  MDD -7.07%  칼마 3.351   ← 종전
    #     E5 ≥10%→0.7    CAGR 23.83%  샤프 **2.119**  MDD -7.07%  칼마 **3.372**  ← 채택
    #  [정확도 열 — 이번 채택의 결정적 근거. 이보다 깨끗한 모양은 없다]
    #     상승 적중 기저대비 +2.99%p → **+2.99%p (완전 동일)**
    #     하락 적중 기저대비 +7.68%p → **+7.68%p (완전 동일)**
    #     놓친 큰상승        60일  → **60일 (완전 동일)**
    #     큰상승일 평균노출   0.4073 → 0.4036 (-0.0037, 무시 가능)
    #     **큰하락일 평균노출 0.3991 → 0.3855 (-0.0136 — 하락 노출만 줄어든다)**
    #  즉 **예측을 하나도 바꾸지 않고, 놓친 상승도 하나도 늘리지 않으면서, 하락 노출만 줄인다.**
    #  샤프 2.119는 이 리포트 06c 전체에서 가장 높은 값이다.
    ...
    #
    # [v1.47.0 §A ⚠⚠ E3 → E7 — 되돌림의 되돌림. v1.46.0 분석에 **측정 범위의 구멍**이 있었다]
    #  v1.46.0에서 나는 이 사다리를 **M 단독 지표로만** 검정하고 "모든 단계가 무의미"라고 결론냈다.
    #  수익 쪽은 지금도 그 결론이 맞다(5일 의존). 그런데 **낙폭 쪽을 M에서 재는 것 자체가 불가능**
    #  했다 — M의 MDD는 2020 코로나 구간이 고정하고 있고, 그 구간은 이격 국면이 아니라서 헤어컷이
    #  아예 닿지 않는다(E1~E13 전부 -7.07%로 완전히 동일). **변하지 않는 숫자로는 효과를 잴 수 없다.**
    #  리포트56에서 같은 사다리를 **사용자가 실제로 운용하는 섹터 층**에서 재보니 정반대였다:
    #     M헤어컷   섹터 CAGR  섹터 MDD    칼마    2024-07~08 구간   13f ②(≤1%p)
    #     off       32.89%   **-15.50%**  2.122   -14.92%
    #     E1        33.85%    -13.37%    2.532   -12.77%
    #     E3(v1.46) 34.78%    -11.21%    3.104   -10.59%          **1.69%p FAIL**
    #     E5        35.26%     -9.85%    3.578    -9.23%           0.34%p PASS
    #     E7        35.71%   **-9.75%**  3.662    -7.87%           0.23%p PASS  ← 채택
    #     E9        36.15%     -9.75%    3.707    -6.50%           0.23%p PASS
    #     E11       36.57%     -9.75%    3.751    -5.11%
    #  이 층은 E_t를 XLK 베타(~1.3)로 증폭하므로 **M의 노출 상한이 곧 이 층의 꼬리**다.
    #  ⚠ E3 되돌림이 실제로 해를 끼쳤다: 섹터 층의 사전등록 수용기준 **13f ②(대조군A 대비 MDD
    #  악화 ≤ 1%p)가 FAIL로 넘어갔다**(악화 1.69%p). 규칙을 다시 조이면 PASS로 돌아온다.
    #  [왜 E7에서 멈추는가 — 이번에는 포화점이 있다]
    #  E7부터 섹터 최대낙폭 **구간이 바뀐다**: 2024-07-10~08-07(엔캐리 청산) → 2019-04-23~06-03.
    #  그리고 E7·E9·E11이 전부 -9.75%로 **완전히 같다** — 헤어컷이 걷어낼 수 있는 낙폭을 다 걷어낸
    #  지점이다. v1.40~v1.45의 CAGR 사다리는 **끝없이 올라가서 멈출 곳이 없었고**, 이 낙폭 사다리는
    #  **정확히 E7에서 멈춘다.** 그 차이가 이번 채택과 지난 세 번의 채택을 가른다.
    #  ⚠ 수익 근거는 여전히 없다(E7의 ★대비 초과 +0.579 → 상위5일 제외 -0.011). **채택 근거는
    #  오직 낙폭이다.** CAGR은 잃지도 얻지도 않는 것으로 본다(기준 ①은 '잃지 않을 것'이므로 통과).
    #  근사 검증: 이 재스케일 방식으로 만든 E9 행(섹터 CAGR 36.15% / MDD -9.75% / 칼마 3.707)이
    #  E9가 라이브였던 **리포트33 실측(36.05% / -9.75% / 3.697)과 거의 일치**한다.
    #  → 앞으로 이 축은 **섹터 층의 [M헤어컷격자]가 매 실행 판정**한다(S v0.34.0).
    #  되돌리려면 ((0.10, 0.8), (0.12, 0.6))[E3, v1.46.0] 또는 ((0.10, 0.7), (0.12, 0.5))[E5].
    ...
    # [v1.60.0 R92 ⚠⚠ 위험 파라미터] 얕은 단계(이격 ≥10%) 0.6 → **0.55** · 깊은 단계 0.0 그대로. 사용자 선택 A안(2026-09-24).
    #   S v0.73.0의 중립채움 1.0→0.5와 **한 묶음**이다: 둘 다 '1999년 이후에만 통하던 규칙'을 긴 이력 쪽으로 되돌리는 방향이며
    #   (채움↓ = 블록 H 지지 방향), 얕은 단계만 블록 J가 지목한 비대칭 1위 버킷(4.6배)이라 한 칸만 깊게 했다.
    #   엔진 s20 하네스(재현 확인): S★ 69.18/92.33 → **70.47/90.41** · I★ 68.92/95.92 → **70.24/93.90**(둘 다 높음 · 여유 0.2%p).
    #     배수 S 19.39→18.79 · I 22.79→22.08 · MDD 동일 −9.85% · 평균 총노출 0.569→0.544(줄어든다).
    #   긴 이력 추정(블록 G·H 기울기 합성 · 현행 대비 Δ(회피+참여)): 1927~49 +0.21 · 1950~98 +1.37 · 1999~2017 +0.11 · 2018~ +0.89%p.
    #   ⚠ 되돌리기(한 줄): m_overrides={"EXTENSION_HAIRCUT_STEPS": ((0.10, 0.6), (0.12, 0.0))} ⇒ v1.59.0과 비트 동일.
    EXTENSION_HAIRCUT_STEPS: Tuple[Tuple[float, float], ...] = ((0.10, 0.55), (0.12, 0.0))   # [v1.60.0 R92] · [v1.59.0 R91] · [v1.58.0] E7→E11
...
def generate_signals(score_pct: pd.Series, trend200: pd.Series, cfg: Config = CFG,
    ...
    if cfg.USE_NEUTRAL_RISK_CUT and cfg.USE_HAZARD_TRACK and haz_pct is not None:
        ...
        if getattr(cfg, "NEUTRAL_RISK_CUT_EXEMPT_PROMO", False):
            _scope = str(getattr(cfg, "NEUTRAL_RISK_CUT_EXEMPT_SCOPE", "all") or "all")
            _promo = (deep_boost.fillna(False).astype(bool)
                      | struct_boost.fillna(False).astype(bool))
            if _scope != "deep":      # "all" = 규칙 ⑤(회복 승격)까지 포함
                _promo = _promo | recovery_floor.fillna(False).astype(bool)
...
#     (개발 중 이 순서를 반대로 뒀다가 ⑩의 발동 플래그가 달라지는 것을 전용 테스트가 잡아냈다).
    ...
    log("SIGNAL", kv(event="generated", switches=n_sw,
                     ...
                     neutral_cut_exempt_scope=str(getattr(cfg, "NEUTRAL_RISK_CUT_EXEMPT_SCOPE", "all")),
...
def _grid_robust(b: pd.DataFrame, base: Optional[pd.DataFrame], topk: int = 5) -> dict:
    ...
    [v1.47.0 ⚠ 기준 ④ 정밀화 — 무엇을 근거로 채택하느냐에 따라 관문이 다르다]
    v1.46.0에서 이 게이트를 만들자마자 그 한계가 드러났다. 게이트는 **수익(초과수익)** 만 본다.
    그런데 어떤 규칙은 수익이 아니라 **낙폭**을 사러 쓴다. 과열 헤어컷이 정확히 그랬다 —
    수익 근거는 5일 잡음이지만(그래서 v1.46.0에서 E3로 되돌렸다) 낙폭 효과는 실재했고,
    **M 단독 지표로는 그 낙폭 효과를 잴 수 없었다**(M의 MDD는 2020 코로나 구간이 고정해
    E1~E13 전부 -7.07%로 동일). 섹터 층에서 재보니 -15.50% ~ -9.75%로 크게 움직였다.
    → 기준 ④를 둘로 나눈다:
      **④-a (수익 근거 채택)**: ★ 대비 초과수익의 부호가 상위 5일 제외 후에도 유지될 것.
      **④-b (낙폭 근거 채택)**: (i) 낙폭 개선이 단조이고 **격자 안에서 포화**할 것 —
         포화점이 없으면 멈출 곳이 없다는 뜻이므로 채택하지 않는다.
         (ii) 초과수익이 상위 5일 제외 후 **음수가 아닐 것**(수익을 강건하게 잃지는 않을 것).
         (iii) **실제로 운용하는 층에서 측정할 것** — E_t처럼 두 층이 공유하는 파라미터는
         상위 층(M) 단독 지표가 그 효과에 **구조적으로 둔감할 수 있다.**
    (iii)은 v1.47.0의 교훈이다. 섹터 층 13_섹터배분전략의 `[M헤어컷격자·근사]`가
    그 판정을 매 실행 자동으로 보여준다(S v0.34.0).

    [v1.48.0 ⚠ ④-b(i) 보강 — '어떤 낙폭 지표로 포화를 보느냐'가 새 손잡이가 되면 안 된다]
    리포트57에서 드러난 함정: 같은 사다리도 **낙폭 지표를 넓힐수록 포화점이 뒤로 밀린다.**
      섹터 층 M헤어컷 사다리 — MDD는 **E7**에서 포화(-9.75%가 E7~E13 동일),
      상위3 낙폭평균은 **E9**에서 포화(-8.99%가 E9~E13), 상위5는 E11, **얼서는 끝내 포화 안 함**.
    지표를 고르는 자유를 두면 "포화했다"는 판정 자체가 튜닝 가능해진다(극단에서는 노출 0).
    → **④-b(i)의 포화는 수용기준이 실제로 쓰는 지표(MDD)로 판정한다.**
      13f ②도 기준 ③도 MDD를 쓰므로, 포화도 MDD로 본다. `상위3 낙폭평균`은 "MDD가 단일 사건에
      고정돼 헛포화한 것은 아닌가"를 교차 확인하는 **보조**이고, `얼서지수`는 **맥락 전용**이다.

    계산: 일별 초과 = (후보 전략수익 - ★ 전략수익). 연율 환산은 산술 합 × 252/N.
    """
    out = {"★대비 초과(연율%p)": np.nan, "초과(상위5일 제외,연율%p)": np.nan,
           "강건성(기준④)": "", "상위3 낙폭평균": np.nan, "얼서지수": np.nan}
    try:
        if b is None or "strategy_ret" not in b.columns:
            return out
        # [v1.48.0 §C] 낙폭을 **하나의 사건에 묶이지 않게** 두 지표로 더 싣는다(진단용).
        #   · 상위3 낙폭평균: 독립 낙폭 에피소드 중 깊은 셋의 평균. MDD가 단일 사건에 고정될 때
        #     '정말 포화했는가'를 교차 확인한다.
        #   · 얼서지수: 낙폭 경로 전체의 RMS. 노출을 줄이면 **언제나** 좋아지므로 **판정에는 쓰지
        #     않는다**(포화점이 없다 = 멈출 곳이 없다). 맥락용으로만 싣는다.
        _r = pd.to_numeric(b["strategy_ret"], errors="coerce").fillna(0.0)
        if len(_r) >= 60:
            _eq = (1.0 + _r).cumprod()
            _dd = _eq / _eq.cummax() - 1.0
            _under = _dd < -1e-9
            _eps, _st = [], None
            for _t, _u in _under.items():
                if _u and _st is None:
                    _st = _t
                if (not _u) and _st is not None:
                    _eps.append(float(_dd.loc[_st:_t].min())); _st = None
            if _st is not None:
                _eps.append(float(_dd.loc[_st:].min()))
            if _eps:
                out["상위3 낙폭평균"] = round(float(np.mean(sorted(_eps)[:3])), 4)
            out["얼서지수"] = round(float(np.sqrt((_dd ** 2).mean())), 4)
        if base is None:
            return out
...
def _grid_accuracy(b: pd.DataFrame) -> dict:
    ...
    **수익률 숫자만 높게 나온다고 좋은 게 아니다**"였다.
    ...
    [v1.45.0 §C ⚠ 측정 결함 수정 — 이진 절단이 사이징 규칙을 구분하지 못했다]
    위 두 적중률은 `pe >= 0.5`로 보유/미보유를 **이진 판정**한다. 리포트54에서 그 한계가
    드러났다: 해어컷 E7(상한 0.6/0.4)과 E9(0.5/0.3)의 적중률 세 열이 **소수점까지 동일**하게
    찍혔다(상승 +4.57 · 하락 +7.39 · 놓친 57). 0.6과 0.5는 둘 다 '보유', 0.4와 0.3은 둘 다
    '미보유'로 같은 쪽에 떨어지기 때문이다. 사이징을 0.1 단위로 조절하는 규칙(⑩ 해어컷,
    ⑨ 중립감축, ⑮ 폭조절)을 이 열들로 비교하면 **차이가 보이지 않거나 절단 위치가 바뀌는
    쪽만 과장**된다. E5→E9에서 하락적중이 8.93 → 7.39로 떨어진 것도 상당 부분 여기서 온다.

    → 기존 열은 **리포트 간 비교 연속성을 위해 그대로 두고**, 노출을 가중치로 쓰는 열을
      나란히 추가한다(이진 절단 없음):
        · 상승적중(노출가중, %p) = Σ(pe·1[r>0])/Σ(pe) − P(r>0)
          "투입한 자본 1단위가 상승일에 있었을 확률"의 기저 대비 초과.
        · 하락적중(노출가중, %p) = Σ((1−pe)·1[r≤0])/Σ(1−pe) − P(r≤0)
          "빼둔 자본 1단위가 하락일에 있었을 확률"의 기저 대비 초과. pe>1이면 1로 클립해
          현금 몫이 음수가 되지 않게 한다(레버리지 격자 행 보호).
        · 놓친 큰상승(노출가중, 일) = Σ over 큰상승일 (1 − pe) — 절단이 아니라 '얼마나
          안 담았는가'의 합. 이진 카운트와 달리 0.5와 0.49를 다르게 센다.
      세 열 모두 **노출이 낮아지면 자동으로 좋아지는 정규화가 아니다** — 상승쪽은 분모가
      투입자본, 하락쪽은 분모가 현금이라 한쪽을 키우면 다른 쪽이 반드시 줄어든다.
    """
    out = {"상승적중 기저대비(%p)": np.nan, "하락적중 기저대비(%p)": np.nan,
           "상승적중(노출가중,%p)": np.nan, "하락적중(노출가중,%p)": np.nan,
           "큰상승일 평균노출": np.nan, "큰하락일 평균노출": np.nan,
           "놓친 큰상승일수": np.nan, "놓친 큰상승(노출가중,일)": np.nan,
           "평균노출(정확도용)": np.nan}
    try:
        ...
        # [v1.45.0 §C] 노출가중 판(이진 절단 없음). pe는 [0,1]로 클립해 현금 몫을 음수로 만들지 않는다.
        w_in = pe.clip(lower=0.0, upper=1.0)
        w_out = 1.0 - w_in
        if float(w_in.sum()) > 1e-9:
            out["상승적중(노출가중,%p)"] = round(
                (float((w_in * (r > 0)).sum() / w_in.sum()) - base_up) * 100, 2)
        if float(w_out.sum()) > 1e-9:
            out["하락적중(노출가중,%p)"] = round(
                (float((w_out * (r <= 0)).sum() / w_out.sum()) - (1.0 - base_up)) * 100, 2)
        out["놓친 큰상승(노출가중,일)"] = round(float(w_out[r >= hi].sum()), 2)
    except Exception:
        pass
    return out
...
def threshold_sensitivity(score_pct: pd.Series, trend200: pd.Series, price: pd.DataFrame,
    ...
    #   불필요하다는 뜻이다).
    # [v1.44.0 §C] PCT_RISK_ON은 네 값이 모든 off에서 소수점까지 동일했다(규칙 ⑥·②가 흡수).
    #   24조합 → 14조합으로 줄이되 **0.40과 0.60 두 행은 canary로 남긴다** — 다시 달라지면
    #   규칙 스택이 바뀐 것이므로 회귀 테스트가 그때 잡는다.
    for off in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35]:
        for on in [0.40, 0.60]:
            if on <= off:
                continue
    ...
    if cfg.USE_RECOVERY_FLOOR:
        ...
        rf_rows = []
        for cpct in [0.015, 0.02, 0.03, 0.04, 0.05, 0.07, 0.10]:   # [v1.37.0] 0.02/0.04 · [v1.41.0] 0.015(채택값 너머 경계) 추가
    ...
    if cfg.USE_TREND_PROMOTION and cfg.USE_HAZARD_TRACK and haz_pct is not None:
        ...
        tp_rows = []
        # [v1.44.0 §A] PCT_RISK_OFF가 0.20 → 0.10으로 내려가면서 이 하한(0.30)의 근거였던
        #   "위험회피 경계 + 버퍼 0.10"이 "경계 + 버퍼 0.20"이 됐다. 등가값 0.20을 격자에 추가해
        #   다음 실행이 재게 한다(라이브는 0.30 그대로 — 한 라운드 한 파라미터).
        for msp in [0.20, 0.25, 0.30, 0.35, 0.40]:
        ...
        if haz_pct is not None:
            ...
            # [v1.43.0 §B] 6번째 원소 = 제외 범위("all" | "deep"). 라이브는 제외 자체가 off.
            nc_cases = [(False, cfg.NEUTRAL_RISK_CUT_H, cfg.POS_NEUTRAL_HIGH_H, False, "all", "off(중립 0.5 유지)"),
                        (True, 0.70, 0.25, False, "all", "0.25 @ H>0.70"), (True, 0.70, 0.0, False, "all", "0.0 @ H>0.70"),
                        (True, 0.75, 0.0, False, "all", "0.0 @ H>0.75"), (True, 0.80, 0.0, False, "all", "0.0 @ H>0.80"),
                        (True, 0.70, 0.0, True, "all", "0.0 @ H>0.70 · ⑤⑦⑧승격일 제외"),
                        (True, 0.70, 0.25, True, "all", "0.25 @ H>0.70 · ⑤⑦⑧승격일 제외"),
                        (True, 0.70, 0.0, True, "deep", "0.0 @ H>0.70 · ⑦⑧만 제외(⑤ 유지)")]
            for use, th, lvl, exempt, scope, label in nc_cases:
                c = Config(**{**cfg.__dict__, "USE_NEUTRAL_RISK_CUT": use,
                              ...
                              "NEUTRAL_RISK_CUT_EXEMPT_SCOPE": scope})
                ...
                is_live = (use == cfg.USE_NEUTRAL_RISK_CUT
                           ...
                           and (not exempt or scope == str(getattr(cfg, "NEUTRAL_RISK_CUT_EXEMPT_SCOPE", "all")))
        ...
        eh_cases = [(False, cfg.EXTENSION_HAIRCUT_STEPS, cfg.EXTENSION_HAIRCUT_SMOOTH, "off(상한 없음)"),
                    ...
                    (True, ((0.10, 0.6), (0.12, 0.4)), cfg.EXTENSION_HAIRCUT_SMOOTH, "E7: ≥10%→0.6, ≥12%→0.4"),   # [v1.40.0] 채택값 너머를 재기 위한 경계 확장
                    (True, ((0.10, 0.5), (0.12, 0.3)), cfg.EXTENSION_HAIRCUT_SMOOTH, "E9: ≥10%→0.5, ≥12%→0.3"),   # [v1.44.0 §B] → [v1.45.0] 라이브
                    ...
                    (True, ((0.10, 0.6), (0.12, 0.0)), cfg.EXTENSION_HAIRCUT_SMOOTH, "R91: ≥10%→0.6, ≥12%→0.0"),      # [v1.59.0 R91] (R92: 직전 라이브)
                    (True, ((0.10, 0.55), (0.12, 0.0)), cfg.EXTENSION_HAIRCUT_SMOOTH, "R92: ≥10%→0.55, ≥12%→0.0"),    # [v1.60.0 R92] 라이브
        ...
        if recov_conf is not None:
            ...
            # [v1.45.0 §D] v1.44.0 §D의 like-for-like 수정으로 이 격자가 처음 제대로 보였고,
            #   리포트54에서 **-12%가 라이브(-15%)를 CAGR 25.75 vs 25.50% · 칼마 3.644 vs 3.607로
            #   앞섰다**(MDD 동일 -7.07%, 하락적중 +8.95 vs +8.93, 놓친 큰상승 55일 동일).
            #   채택은 '한 커밋 한 신호 파라미터'(v1.4.0 §9) 때문에 이번 라운드에는 미루고,
            #   -12%가 격자의 끝이므로 **-10%·-8%를 심어** 다음 실행이 정점을 확정하게 한다.
            cases += [(t, live_mode, live_guard, f"{t:.0%}") for t in (-0.08, -0.10, -0.12, -0.15, -0.20)]
            ...
            for dd_th, mode, guard, label in cases:
                ...
                # [v1.44.0 §C ⚠ 버그 수정] struct_dd(규칙 ⑧)를 넘기지 않아 이 격자만 규칙 ⑧이
                #   꺼진 채로 재시뮬레이션됐다 — ★ 행이 라이브와 달랐다(리포트47~53 전부).
                #   파일 안 22개 generate_signals 호출부 중 유일한 누락이었다.
                sg = generate_signals(score_pct, trend200, c, haz_pct=haz_pct,
                                      fast_pct=fast_pct, recov_conf=recov_conf,
                                      deep_recov=dr_case, struct_dd=struct_dd,
                                      px=price["Close"], breadth=breadth)
                b = run_backtest(price, sg["target_pos"], c, rf_daily)
                b = b.loc[b.index >= pd.Timestamp(cfg.SIGNAL_START)]
                m = perf_metrics(b["strategy_ret"])
...
def run(cfg: Config = CFG) -> dict:
    ...
    log("CONFIG", kv(event="r92_sizing", bundle=BUNDLE_VERSION, pos_neutral=cfg.POS_NEUTRAL,
                     haircut=";".join(f"{t:.2f}->{c:.2f}" for t, c in _hc_now),
                     r92_ref="0.10->0.55;0.12->0.00", r91_ref="0.10->0.60;0.12->0.00", r88_ref="0.10->0.40;0.12->0.20",
                     is_r92=bool(_hc_now == ((0.10, 0.55), (0.12, 0.0))),
                     r86_ref="pos_neutral=0.50 haircut=0.10->0.60;0.12->0.40",
                     is_r91=bool(_hc_now == ((0.10, 0.6), (0.12, 0.0))),
                     changed_vs_r91=bool(_hc_now != ((0.10, 0.6), (0.12, 0.0))),
...
def build_report(res: dict, cfg: Config = CFG) -> str:
    ...
    meta = [
        ...
        # [v1.49.0] 격자 수렴 상태 한 줄 — 매 라운드 사람이 06c를 손으로 세던 것을 리포트가 직접 답한다.
        #   ① CAGR 손실 ≤ 0.5%p · ② ① 통과자 중 칼마 정점 · ③ MDD 악화 ≤ 0.5%p · ④ 강건성='통과'.
        #   **①②③④ 전부 통과 0행 = 격자 수렴**(이번 실행에 채택할 후보가 없다는 뜻).
        ("격자 수렴 상태(①②③④)", _grid_convergence_line(res)),
...
def _grid_convergence_line(res: dict) -> str:
    """[v1.49.0] 06c 임계값민감도 격자가 수렴했는지 한 줄로 요약한다(진단 전용 — 신호 무변경).

    채택 기준(사전 고정, CHANGELOG v1.46.0/v1.47.0 §④):
      ① ★ 대비 CAGR 손실이 0.5%p를 넘지 않을 것
      ② ①을 통과한 행 중 칼마 정점일 것
      ③ MDD 악화가 0.5%p 이하일 것
      ④ 강건성(기준④) = '통과' — 상위 5일을 빼도 ★ 대비 초과의 부호가 살아남을 것
    네 기준을 **모두** 통과하는 행이 0이면 격자는 수렴했고, 그 라운드의 개선은 격자 채택이
    아니라 새 진단에서 나와야 한다. 매 라운드 손으로 세던 작업을 리포트가 대신한다.
    """
    try:
        g = res.get("sens")
        if g is None or not isinstance(g, pd.DataFrame) or g.empty:
            return "계산불가(06c 없음)"
        need = ("CAGR", "칼마", "최대낙폭", "강건성(기준④)")
        if any(c not in g.columns for c in need):
            return "계산불가(06c 열 부족)"
        t = g[g["CAGR"].notna() & g["칼마"].notna() & g["최대낙폭"].notna()].copy()
        st = t[t["강건성(기준④)"].astype(str).eq("★")]
        if not len(st):
            return "계산불가(★ 행 없음)"
        sc = float(st["CAGR"].iloc[0]); sm = float(st["최대낙폭"].iloc[0]); sk = float(st["칼마"].iloc[0])
        c1 = t["CAGR"].astype(float) >= sc - 0.005
        c3 = t["최대낙폭"].astype(float) >= sm - 0.005
        c4 = t["강건성(기준④)"].astype(str).eq("통과")
        peak = float(t.loc[c1, "칼마"].astype(float).max()) if int(c1.sum()) else float("nan")
        c2 = t["칼마"].astype(float) >= (max(sk, peak) if peak == peak else sk)
        allp = t[c1 & c2 & c3 & c4]
        nm = "없음"
        if len(allp):
            # 06c는 격자 블록마다 자기 파라미터 열만 채운다 — 그 행에서 값이 있는 파라미터 열로 후보를 식별한다.
            _skip = set(need) | {"투자시간비율", "거래횟수", "평균노출(정확도용)", "총수익배수",
                                 "★대비 초과(연율%p)", "초과(상위5일 제외,연율%p)", "상위3 낙폭평균", "얼서지수"}
            _r0 = allp.iloc[0]
            _who = [f"{c}={_r0[c]}" for c in allp.columns
                    if c not in _skip and pd.notna(_r0[c]) and str(_r0[c]).strip() not in ("", "해당없음")]
            nm = (" · ".join(_who[:3]) or "행 %d" % int(allp.index[0]))[:60]
        return (f"격자 {len(t)}행 — ① 통과 {int(c1.sum())} · ④ 통과 {int(c4.sum())} · "
                f"**①②③④ 전부 통과 {len(allp)}행** (후보: {nm}) | "
                f"★ CAGR {sc:.2%} 칼마 {sk:.3f} MDD {sm:.2%} · 격자 칼마 정점 "
                f"{(f'{peak:.3f}' if peak == peak else '-')} — 0행이면 이번 실행에 채택 후보 없음")
