...
#    · 갱신 확인 문구를 "23·24 시트 + 10_데이터품질 [신선도] 행"으로 바꿨다.
#
...
def main(sector_exclude: Optional[Tuple[str, ...]] = None, run_industry_layer: bool = True,
    ...
    *_overrides: 각 Config 필드 덮어쓰기(dataclasses.replace) — 예: s_overrides={"MAX_WORKERS": 2}.
    ...
    for _fn, (_tag, _min, _mod) in _MIN.items():
        ...
        # [v1.21.0 ROUND67] 데이터 신선도 가드 · 예측품질 검정 — 사용자 지적 2건의 확인 지점
        if hasattr(_mod, "ensure_fresh_sector_prices"):
            _feat.append("★데이터신선도가드")
        if hasattr(_mod, "build_prediction_quality"):
