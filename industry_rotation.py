...
#    (§2) _i_relcmp_diag가 넘기는 S 값(m_proxy_long · m_proxy_long_ff · long)에 R99 항목(guard_live_off · V1S · m5_verdict)이 실려
#         I 00 '★★★ R99 판정' · '★★★ R99 I 높음 경로(M5)' · 'R99 N6-d — 산업 층은 선택 개선을 시도하지 않는다'(방법서 N6-d) 줄과 00U 블록 M⑧·N⑧·F2 M5가 나온다.
#    (§3) K 통로 sector_w_variants 필터는 그대로(R95 · R96 · R98 V1 · R98 VRP(M이 끄면 없음) · R98 상한100% = M5) — K v0.11.0 비교 행.
#    (§4) N6-d: 산업 층(부모 안) 선택 정보 0(모든 신호 |t| < 0.9) — I 선택 규칙은 바꾸지 않는다(부모 ETF 주차 + 리더 확신 게이트 유지).
#    연구·교육용이며 투자 자문이 아니다.
...
#    (§2) _i_relcmp_diag에 r96 · r96_long → I 00 'R96 라이브' 줄 · 00U 블록 L(R96 판정 + R95 규칙별 몫). I 비교 행은 S relcmp_frames로 자동.
#    (§3) K 통로 sector_w_variants에 S 'R96 변동성 관리 없음(= R95 M)' 섹터 비중도 싣는다(K v0.9.2 비교 행).
#    연구·교육용 — 투자 자문이 아니다.
